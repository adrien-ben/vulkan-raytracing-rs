...
impl App {
    fn new(window: &Window) -> Result<Self> {
        ...
        let vertex_buffer_info = vk::BufferCreateInfo::builder()
            .size(size_of_val(&VERTICES) as _)
        ...
        unsafe {
            let data_ptr = vertex_allocation.mapped_ptr().unwrap().as_ptr();
            let mut align = ash::util::Align::new(
                data_ptr,
                align_of::<Vertex>() as _,
        ...
        let index_allocation = allocator.allocate(&AllocationCreateDesc {
            name: "Indices",
        ...
        unsafe {
            let data_ptr = index_allocation.mapped_ptr().unwrap().as_ptr();
            let mut align =
                ash::util::Align::new(data_ptr, align_of::<u16>() as _, size_of_val(&INDICES) as _);
...
impl Drop for App {
    fn drop(&mut self) {
        unsafe {
            ...
            allocator
                .free(self.vertex_allocation.take().unwrap())
                .unwrap();
            self.device.destroy_buffer(self.vertex_buffer, None);
            allocator
                .free(self.index_allocation.take().unwrap())
                .unwrap();
...
fn create_and_record_command_buffers(
    ...
    for (index, buffer) in buffers.iter().enumerate() {
        ...
        unsafe { device.cmd_bind_vertex_buffers(buffer, 0, &[vertex_buffer], &[0]) };
        ...
        unsafe { device.cmd_draw_indexed(buffer, 3, 1, 0, 0, 0) };
